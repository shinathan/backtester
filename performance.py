...
def calculate_time_in_market(portfolio_log):
    ...
    return round((portfolio_log['positions_value'] / portfolio_log['equity']).mean() * 100, 0)
...
def calculate_fees_drag(portfolio_log, fills_log):
    ...
    portfolio_log_daily = portfolio_log_daily.merge(fills_log_daily, left_index=True, right_index=True, how='left')
...
def display_stats(portfolio_log, fills_log, trade_log):
    ...
    statistics = {
        ...
        "Sharpe": calculate_sharpe(portfolio_log),
        "Sortina": calculate_sortina(portfolio_log),
        ...
        "Profit factor": calculate_profit_factor(trade_log),
        "Trades/month": calculate_trades_per_month(portfolio_log, trade_log),
...
def create_daily_portfolio_log(portfolio_log):
    ...
    market_dates_portfolio = list(filter(lambda day: (day >= portfolio_log.index[0]) & \
        (day <= portfolio_log.index[-1]), market_dates ) )

    portfolio_log = portfolio_log.reindex(market_dates_portfolio)
    portfolio_log[['equity', 'cash', 'return_cum']] = portfolio_log[['equity', 'cash', 'return_cum']].ffill()
